...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!member.bannable) return message.reply("I cannot ban this user! Do they have a higher role? Do I have ban permissions?").then(msg => msg.delete({timeout:10000}));

        let reason = args.slice(1).join(' ');
        if (!reason) reason = "No reason provided";

        await member.ban(reason)
            .catch(error => message.reply(`Sorry ${message.author} I couldn't ban because of : ${error}`).then(msg => msg.delete({timeout:10000})));
