...
module.exports = {
	...
	async execute(message, args) {
		try {
			...
			if (now - prevPingCmd <= 8000)	return message.reply('OOps, a little bit too quick there!').then(msg => msg.delete(3000));
			...
			else if (args.length && args[0] == 'bot') return message.channel.bulkDelete(fetched.fiilter(auth => auth.author.bot == true));
