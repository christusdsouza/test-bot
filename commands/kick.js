...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!args.length) return message.reply("Looks like you forgot the one who you wanted to kick in the balls").then(msg => msg.delete({timeout:10000}));
        ...
        if (!member) return message.reply("Please mention a valid member of this server").then(msg => msg.delete({timeout:10000}));
        if (!member.kickable) return message.reply("I cannot kick this user! Do they have a higher role? Do I have kick permissions?").then(msg => msg.delete({timeout:10000}));
        ...
        await member.kick(reason)
            .catch(error => message.reply(`Sorry ${message.author} I couldn't kick because of : ${error}`).then(msg => msg.delete({timeout:10000})));
